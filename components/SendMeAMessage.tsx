...
export default function SendMeAMessage() {
    ...
    return(
        <div id="contact" className="grid grid-cols-1 grid-rows-[1fr_9fr_1fr] h-screen bg-primary font-inconsolata px-4 py-4">
            <motion.div initial={{opacity: 0, y: 40}} whileInView={{opacity: 1, y: 0}} transition={{duration: 0.5, ease: easeOut}} className="bg-coolgreen text-black text-[clamp(1.5rem,3vw,5rem)] w-fit p-1 flex items-center font-extrabold shadow-xl">
                Send Me a Message
            </motion.div>
            <motion.div initial={{opacity: 0, y: 40}} whileInView={{opacity: 1, y: 0}} transition={{duration: 0.5, ease: easeOut, delay: 0.25}} className="flex justify-center items-center">
                <form onSubmit={handleSubmit} className="border border-black max-sm:h-[clamp(70px,50vh,700px)] max-md:h-[clamp(70px,55vh,700px)] h-[clamp(70px,70vh,700px)] w-[clamp(100px,90vw,500px)] bg-form grid grid-rows-[1fr_1fr_1fr_1fr] grid-cols-1 p-4 shadow-xl leading-none gap-2">
                    ...
                    <motion.button whileTap={{scale: 0.9, y: 3}} whileHover={{scale: 1.05}} type="submit" className="bg-coolgreen rounded-full w-[clamp(2rem,20vw,10rem)] text-black h-[clamp(2rem,5vw,4rem)] justify-self-center self-center shadow-xl border border-black col-span-2 cursor-pointer flex items-center justify-center gap-4">
                        Send 
                        <Image alt="send-icon" src="/images/send_4077212.png" width={50} height={50} className="w-[30px] h-[30px]" />
                    </motion.button>
                </form>
            </motion.div>
            <motion.div initial={{opacity: 0, y: 40}} whileInView={{opacity: 1, y: 0}} transition={{duration: 0.5, ease: easeOut, delay: 0.25}} className="bg-form max-md:flex max-md:flex-col max-md:text-center grid grid-rows-2 grid-cols-1 text-[clamp(1rem,1.5vw,5rem)] shadow-xl text-black">
                <div className="row-start-1 flex justify-evenly max-md:flex-col">
                    ...
                    <div className="">
                        ...
                        <motion.a whileHover={{scale: 1.5}} href="https://github.com/gsincodes">Github</motion.a>
                    </div>
                </div>
